...
public class SubPerProgramScreen {
    ...
    public static void display() {
        ...
        for (String line : token) {
            ...
            if (parts.length >= 3) {
                program = parts[2].trim(); // "Yoga" from "John : Statham : Yoga : 3 months"
            } else if (parts.length == 2) {
                program = parts[1].trim(); // "ProgramC" from "User4 : ProgramC"
            } else if (parts.length == 1) {
                program = parts[0].trim(); // fallback if line is just "Yoga"
            }
